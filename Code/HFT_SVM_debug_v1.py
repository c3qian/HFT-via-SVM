...
for name in name_list:
	AMZN_msg_df = pd.read_csv('AMZN_message_10.csv',names = ['Time (sec)',
		'Event Type',
		'Order ID',
		'Size',
		'Price',
		'Direction'])
	AMZN_odr_df = pd.read_csv('AMZN_orderbook_10.csv',
						names = [f'{ty}.{VP}.{i}' 
							for i in range(1,11) for ty in ['ASK','BID'] 
								for VP in ['Price','Size'] ])
	AMZN_odr_df['Time (sec)'] = AMZN_msg_df['Time (sec)']

	AAPL_msg_df = pd.read_csv('AAPL_message_10.csv',names = ['Time (sec)',
		'Event Type',
		'Order ID',
		'Size',
		'Price',
		'Direction'])
	AAPL_odr_df = pd.read_csv('AAPL_orderbook_10.csv',
						names = [f'{ty}.{VP}.{i}' 
							for i in range(1,11) for ty in ['ASK','BID'] 
								for VP in ['Price','Size'] ])
	AAPL_odr_df['Time (sec)'] = AAPL_msg_df['Time (sec)']

	GOOG_msg_df = pd.read_csv('GOOG_message_10.csv',names = ['Time (sec)',
		'Event Type',
		'Order ID',
		'Size',
		'Price',
		'Direction'])
	GOOG_odr_df = pd.read_csv('GOOG_orderbook_10.csv',
						names = [f'{ty}.{VP}.{i}' 
							for i in range(1,11) for ty in ['ASK','BID'] 
								for VP in ['Price','Size'] ])
	GOOG_odr_df['Time (sec)'] = GOOG_msg_df['Time (sec)']

	INTC_msg_df = pd.read_csv('INTC_message_10.csv',names = ['Time (sec)',
		'Event Type',
		'Order ID',
		'Size',
		'Price',
		'Direction'])
	INTC_odr_df = pd.read_csv('INTC_orderbook_10.csv',
						names = [f'{ty}.{VP}.{i}' 
							for i in range(1,11) for ty in ['ASK','BID'] 
								for VP in ['Price','Size'] ])
	INTC_odr_df['Time (sec)'] = INTC_msg_df['Time (sec)']
	MSFT_msg_df = pd.read_csv('MSFT_message_10.csv',names = ['Time (sec)',
		'Event Type',
		'Order ID',
		'Size',
		'Price',
		'Direction'])
	MSFT_odr_df = pd.read_csv('MSFT_orderbook_10.csv',
						names = [f'{ty}.{VP}.{i}' 
							for i in range(1,11) for ty in ['ASK','BID'] 
								for VP in ['Price','Size'] ])
	MSFT_odr_df['Time (sec)'] = MSFT_msg_df['Time (sec)']
...
time_series = msg_pack[name]['Time (sec)'].copy()
...
# %%
# count number of upward mid price movement :13680, and 15398 for spread crossing
label_pack['AMZN']['MID.Dir'][label_pack['AMZN']['MID.Dir']>0].count()
...
# %%
def slice_label_pack(randomList,label_pack, metric):
    '''
    metic: Spread, MidPrice
    return a pd.Series sorted by time
    '''
    if metric =='Spread':
        temp = list(label_pack[name].iloc[randomList]['Spread.crossing'])
        return pd.Series(temp,index = label_pack[name]['Time (sec)'].iloc[randomList]).sort_index()
    elif metric == 'MidPrice':
        temp = list(label_pack[name].iloc[randomList]['MID.Dir'])
        return pd.Series(temp,index = label_pack[name]['Time (sec)'].iloc[randomList]).sort_index()
    else:
        raise ValueError
...
label_df_slice = slice_label_pack(randomList,label_pack,'MidPrice')
...
# %%
featuresAndLabels.loc['Label'].value_counts()
...
X_train = test_features.drop(['Label'],axis = 1)
Y_train_bin = lb.fit_transform(test_features['Label'])
Y_train = test_features['Label']


# %%
test_features['Label'].value_counts ()
...
# %%
# SVM construction
poly_svm = svm.SVC(kernel='polynomial')
poly_svm.fit(v6, label_df_slice)
